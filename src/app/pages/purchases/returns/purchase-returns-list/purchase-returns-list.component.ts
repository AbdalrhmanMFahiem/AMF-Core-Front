...
@Component({
  ...
  template: `
    <app-page-breadcrumb [pageTitle]="'purchaseReturns.title'" />
    <div class="space-y-6">
      ...
      <app-crud-list
        [pageTitle]="'purchaseReturns.list'"
        [columns]="columns"
        [data]="data"
        [isLoading]="loading"
        [searchPlaceholder]="'Common.SearchPlaceholder'"
        addBtnText="purchaseReturns.add"
        [filters]="filters"
        ...
        <div advanced-filters class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          <div>
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ 'purchaseReturns.fields.status' | translate }}</label>
...
export class PurchaseReturnsListComponent implements OnInit {
  ...
  customActions = [
    {
      id: 'pay',
      label: 'purchaseReturns.addPayment',
  ...
  columns: CrudColumn[] = [
    { field: 'code', header: 'purchaseReturns.fields.code', type: 'code' },
    { field: 'businessPartnerName', header: 'purchaseReturns.fields.businessPartner', type: 'text' },
    { field: 'invoiceDate', header: 'purchaseReturns.fields.invoiceDate', type: 'date' },
    { field: 'totalAmountDisplay', header: 'purchaseReturns.fields.totalAmount', type: 'text' },
    { field: 'remainingAmountDisplay', header: 'purchaseReturns.fields.remainingAmount', type: 'text' },
    { field: 'statusDisplay', header: 'purchaseReturns.fields.status', type: 'dynamic-badge' }
  ];
  ...
  loadData(): void {
    this.loading = true;
    this.invoiceService.getReturns(this.filters, 'purchases').subscribe({
      next: (res) => {
    ...
    this.invoiceService.getReturnStats(this.filters, 'purchases').subscribe({
      next: (res) => {
        this.stats = res;
  ...
  onAdd(): void {
    this.router.navigate(['/purchases/returns/add']);
  }

  onView(id: number): void {
    this.router.navigate(['/purchases/returns/view', id]);
  }
