...
export class PurchaseReturnFormComponent implements OnInit, HasUnsavedChanges {
  ...
  originalInvoicesOptions: SearchableOption[] = [];
  returnableItems: ReturnableItemResponse[] = [];
  ...
  loadInitialData(): void {
    ...
    const invoices$ = this.invoiceService.getPurchaseInvoicesLookup();
    const action$ = this.id ? this.invoiceService.get(this.id, 'purchases') : this.invoiceService.getReturnNextCode('purchases');
    ...
    forkJoin({
      ...
      next: (res: any) => {
        ...
        this.originalInvoicesOptions = res.invoices.map((i: any) => ({ value: i.id, label: i.name }));

        if (this.id) {
          this.processViewResponse(res.actionData as InvoiceResponse);
          if (this.model.originalInvoiceId) {
            this.isLinkedReturn = true;
          }
        } else {
  ...
  processViewResponse(res: InvoiceResponse): void {
    ...
    this.model = {
      ...
      originalInvoiceId: res.originalInvoiceId
    };
  ...
  loadRecord(id: number): void {
    this.loading = true;
    this.invoiceService.get(id, 'purchases').subscribe({
  ...
  onLinkedReturnChange(): void {
    if (this.mode === 'view') return;
    this.model.originalInvoiceId = undefined;
  ...
  onOriginalInvoiceChange(): void {
    if (this.mode === 'view' || !this.model.originalInvoiceId) return;
    
    this.loading = true;
    this.invoiceService.get(this.model.originalInvoiceId, 'purchases').subscribe({
      next: (invoice) => {
        this.model.businessPartnerId = invoice.businessPartnerId;
        this.model.warehouseId = invoice.warehouseId;
        
        this.invoiceService.getReturnableItems(this.model.originalInvoiceId!, 'purchases').subscribe({
          next: (items) => {
            this.returnableItems = items;
            // auto-fill lines with available items
            this.model.lines = items.map(item => ({
              itemId: item.itemId,
              ...
              _discountFixedMode: 'percentage',
              _taxFixedMode: 'percentage',
              _availableToReturn: item.availableToReturn
            } as any));
            this.model.lines.forEach((l, i) => l.lineOrder = i + 1);
            this.recalculateTotals();
            this.loading = false;
          },
  ...
  // Items Tab Actions
  openItemModal(): void {
    if (this.mode === 'view' || this.isLinkedReturn) return;
  ...
  onItemSelected(item: ItemLookupResponse): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('purchaseReturns.errors.duplicateItem')];
  ...
  onCostElementSelected(element: InvoiceCostElementDropdown): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('purchaseReturns.errors.duplicateCostElement')];
      setTimeout(() => this.validationErrors = [], 4000);
      return;
    }
  ...
  validate(): boolean {
    ...
    if (!this.model.businessPartnerId) {
      this.validationErrors.push(`${this.translate.instant('purchaseReturns.fields.businessPartner')}: ${this.translate.instant('validation.required')}`);
    }
    if (!this.model.lines || this.model.lines.length === 0) {
      this.validationErrors.push(this.translate.instant('purchaseReturns.errors.atLeastOneItem'));
    } else {
      const invalidQuantity = this.model.lines.some((l: any) => l.quantity <= 0);
      if (invalidQuantity) {
        this.validationErrors.push(this.translate.instant('purchaseReturns.errors.invalidQuantity'));
      }
      const invalidPrice = this.model.lines.some((l: any) => l.unitPrice <= 0);
      if (invalidPrice) {
        this.validationErrors.push(this.translate.instant('purchaseReturns.errors.invalidPrice'));
      }
    }
  ...
  onSubmit(): void {
    ...
    this.invoiceService.addReturn(requestToSend, 'purchases').subscribe({
      next: (res) => {
        this.saving = false;
