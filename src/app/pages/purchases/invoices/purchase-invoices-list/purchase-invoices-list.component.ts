...
@Component({
  ...
  template: `
    <app-page-breadcrumb [pageTitle]="'purchaseInvoices.title'" />
    <div class="space-y-6">
      ...
      <app-crud-list
        [pageTitle]="'purchaseInvoices.list'"
        [columns]="columns"
        [data]="data"
        [isLoading]="loading"
...
export class PurchaseInvoicesListComponent implements OnInit {
  ...
  updateStatusOptions(): void {
    this.statusOptions = [
      { value: 'Draft', label: this.translate.instant('purchaseInvoices.status.Draft') },
      { value: 'Confirmed', label: this.translate.instant('purchaseInvoices.status.Confirmed') },
      { value: 'PartiallyPaid', label: this.translate.instant('purchaseInvoices.status.PartiallyPaid') },
      { value: 'FullyPaid', label: this.translate.instant('purchaseInvoices.status.FullyPaid') },
      { value: 'Cancelled', label: this.translate.instant('purchaseInvoices.status.Cancelled') }
    ];
  }
  ...
  loadData(): void {
    this.loading = true;
    this.invoiceService.getAll(this.filters, 'purchases').subscribe({
      next: (res) => {
    ...
    this.invoiceService.getStats(this.filters, 'purchases').subscribe({
      next: (res) => {
        this.stats = res;
  ...
  onAdd(): void {
    this.router.navigate(['/purchases/invoices/add']);
  }

  onView(id: number): void {
    this.router.navigate(['/purchases/invoices/view', id]);
  }
