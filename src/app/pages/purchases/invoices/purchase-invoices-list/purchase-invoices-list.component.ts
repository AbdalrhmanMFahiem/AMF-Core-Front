  ...
  template: `
    ...
    <app-print-preview-modal 
      *ngIf="isPrintModalOpen"
      ...
      [loading]="pdfLoading"
      [title]="('purchaseInvoices.printInvoice' | translate) + ' ' + (selectedInvoiceForPrint?.code || '')"
...
export class PurchaseInvoicesListComponent implements OnInit {
  ...
  customActions = [
    ...
    {
      id: 'print',
      label: 'purchaseInvoices.printInvoice',
    ...
    if (event.actionId === 'pay') {
      ...
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          ...
          if (result.isConfirmed) {
            this.invoiceService.confirm(event.item.id, 'purchases').subscribe({
              next: () => {
                this.loadData();
              }
            });
      ...
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          ...
          if (result.isConfirmed) {
            this.invoiceService.cancel(event.item.id, 'purchases').subscribe({
              next: () => {
                this.loadData();
              }
            });
      ...
      this.invoiceService.printPdf(event.item.id, 'purchases').subscribe({
        next: (blob) => {
