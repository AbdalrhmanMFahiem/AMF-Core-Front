...
export class PurchaseInvoiceFormComponent implements OnInit, HasUnsavedChanges {
  ...
  onItemSelected(item: ItemLookupResponse): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('purchaseInvoices.errors.duplicateItem')];
  ...
  onCostElementSelected(element: InvoiceCostElementDropdown): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('purchaseInvoices.errors.duplicateCostElement')];
      setTimeout(() => this.validationErrors = [], 4000);
      return;
    }
