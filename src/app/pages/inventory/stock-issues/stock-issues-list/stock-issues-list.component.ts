...
export class StockIssuesListComponent implements OnInit {
  ...
  loadData(): void {
    this.loading = true;
    this.stockVoucherService.getAll(this.pageNumber, this.pageSize, this.searchValue, 1).subscribe({
      next: (res) => {
        this.data = res;
        this.loading = false;
      },
      error: (err) => {
        this.toastr.error('Failed to load stock issues', 'Error');
        console.error(err);
        this.loading = false;
      }
    });
  ...
  onAdd(): void {
    this.router.navigate(['/inventory/stock-issues/add']);
  }

  onView(id: number): void {
    this.router.navigate(['/inventory/stock-issues/view', id]);
  }
