...
@Component({
  ...
  template: `
    ...
    <div class="space-y-6">
      ...
      <app-crud-list
        [pageTitle]="'salesReturns.list'"
        [columns]="columns"
        [data]="data"
        [isLoading]="loading"
        [searchPlaceholder]="'Common.SearchPlaceholder'"
        addBtnText="salesReturns.add"
        [filters]="filters"
        ...
        <div advanced-filters class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          <div>
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ 'salesReturns.fields.status' | translate }}</label>
          ...
          <div>
            ...
            <app-date-picker id="invoiceDateFrom" name="invoiceDateFrom" [(ngModel)]="filters.invoiceDateFrom" (ngModelChange)="loadData()" placeholder="Common.selectDate"></app-date-picker>
          </div>
          <div>
...
export class SalesReturnsListComponent implements OnInit {
  ...
  columns: CrudColumn[] = [
    { field: 'code', header: 'salesReturns.fields.code', type: 'code' },
    { field: 'businessPartnerName', header: 'salesReturns.fields.businessPartner', type: 'text' },
    { field: 'invoiceDate', header: 'salesReturns.fields.invoiceDate', type: 'date' },
    { field: 'totalAmountDisplay', header: 'salesReturns.fields.totalAmount', type: 'text' },
    { field: 'remainingAmountDisplay', header: 'salesReturns.fields.remainingAmount', type: 'text' },
    { field: 'statusDisplay', header: 'salesReturns.fields.status', type: 'dynamic-badge' }
  ];
  ...
  updateStatusOptions(): void {
    this.statusOptions = [
      { value: 'Draft', label: this.translate.instant('salesReturns.status.Draft') },
      { value: 'Confirmed', label: this.translate.instant('salesReturns.status.Confirmed') },
      { value: 'PartiallyPaid', label: this.translate.instant('salesReturns.status.PartiallyPaid') },
      { value: 'FullyPaid', label: this.translate.instant('salesReturns.status.FullyPaid') },
      { value: 'Cancelled', label: this.translate.instant('salesReturns.status.Cancelled') }
    ];
  }
  ...
  loadData(): void {
    this.loading = true;
    this.invoiceService.getReturns(this.filters).subscribe({
      next: (res) => {
        ...
        const mappedItems = res.items.map((item: InvoiceBasicResponse) => ({
          ...item,
          statusDisplay: this.translate.instant('salesReturns.status.' + item.status),
    ...
    this.invoiceService.getReturnStats(this.filters).subscribe({
      next: (res) => {
        this.stats = res;
