...
export class SalesReturnFormComponent implements OnInit, HasUnsavedChanges {
  ...
  loadInitialData(): void {
    ...
    const invoices$ = this.invoiceService.getSalesInvoicesLookup();
    const action$ = this.id ? this.invoiceService.get(this.id) : this.invoiceService.getReturnNextCode('sales');
  ...
  onOriginalInvoiceChange(): void {
    ...
    this.loading = true;
    this.invoiceService.get(this.model.originalInvoiceId).subscribe({
      next: (invoice) => {
        this.model.businessPartnerId = invoice.businessPartnerId;
        this.model.warehouseId = invoice.warehouseId;
        
        this.invoiceService.getReturnableItems(this.model.originalInvoiceId!, 'sales').subscribe({
          next: (items) => {
  ...
  onItemSelected(item: ItemLookupResponse): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('salesReturns.errors.duplicateItem')];
  ...
  onCostElementSelected(element: InvoiceCostElementDropdown): void {
    ...
    if (exists) {
      this.validationErrors = [this.translate.instant('salesReturns.errors.duplicateCostElement')];
      setTimeout(() => this.validationErrors = [], 4000);
      return;
    }
  ...
  validate(): boolean {
    ...
    if (!this.model.businessPartnerId) {
      this.validationErrors.push(`${this.translate.instant('salesReturns.fields.businessPartner')}: ${this.translate.instant('validation.required')}`);
    }
    if (!this.model.lines || this.model.lines.length === 0) {
      this.validationErrors.push(this.translate.instant('salesReturns.errors.atLeastOneItem'));
    } else {
      const invalidQuantity = this.model.lines.some((l: any) => l.quantity <= 0);
      if (invalidQuantity) {
        this.validationErrors.push(this.translate.instant('salesReturns.errors.invalidQuantity'));
      }
      const invalidPrice = this.model.lines.some((l: any) => l.unitPrice <= 0);
      if (invalidPrice) {
        this.validationErrors.push(this.translate.instant('salesReturns.errors.invalidPrice'));
      }
    }
  ...
  onSubmit(): void {
    ...
    this.invoiceService.addReturn(requestToSend, 'sales').subscribe({
      next: (res) => {
        this.saving = false;
