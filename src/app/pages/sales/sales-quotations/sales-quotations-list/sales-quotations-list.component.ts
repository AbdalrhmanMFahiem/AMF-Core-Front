...
import { SalesQuotationBasicResponse, SalesQuotationFilters } from '../../../../core/models/sales-quotation.model';
...
@Component({
  ...
  template: `
    <app-page-breadcrumb [pageTitle]="'salesQuotations.title'" />
    <div class="space-y-6">
      <app-crud-list
        [pageTitle]="'salesQuotations.list'"
        [columns]="columns"
        [data]="data"
        [isLoading]="loading"
        [searchPlaceholder]="'common.searchPlaceholder'"
        addBtnText="salesQuotations.add"
        [filters]="filters"
    ...
    <app-print-preview-modal 
      *ngIf="isPrintModalOpen"
      ...
      [loading]="pdfLoading"
      [title]="('salesQuotations.printOrder' | translate) + ' ' + (selectedItemForPrint?.code || '')"
...
export class SalesQuotationsListComponent implements OnInit {
  ...
  customActions = [
    {
      id: 'confirm',
      label: 'salesQuotations.confirmTitle',
    ...
    {
      id: 'cancel',
      label: 'salesQuotations.cancelTitle',
    ...
    {
      id: 'close',
      label: 'salesQuotations.closeDocument',
      icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>',
    ...
    {
      id: 'print',
      label: 'salesQuotations.printOrder',
  ...
  updateStatusOptions(): void {
    ...
    this.approvalStatusOptions = [
      { value: 'Pending', label: this.translate.instant('salesQuotations.status.Pending') },
      { value: 'Approved', label: this.translate.instant('salesQuotations.status.Approved') },
      { value: 'Rejected', label: this.translate.instant('salesQuotations.status.Rejected') }
    ];
  }
  ...
  loadData(): void {
    this.loading = true;
    this.salesQuotationService.getAll(this.filters).subscribe({
      next: (res: any) => {
        ...
        const mappedItems = itemsList.map((item: SalesQuotationBasicResponse) => ({
          ...item,
          statusDisplay: this.translate.instant('common.documentStatus.' + item.status),
          statusDisplayColor: getStatusColor(item.status),
          approvalStatusDisplay: item.approvalStatus ? this.translate.instant('salesQuotations.status.' + item.approvalStatus) : '-',
  ...
  onAdd(): void {
    this.router.navigate(['/sales/sales-quotations/add']);
  }

  onView(id: number): void {
    this.router.navigate(['/sales/sales-quotations/view', id]);
  }

  onToggleStatus(item: SalesQuotationBasicResponse): void {
    // No operation
  }
  ...
  onCustomAction(event: { actionId: string, item: any }) {
    if (event.actionId === 'confirm') {
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          title: this.translate.instant('salesQuotations.confirmTitle'),
          text: this.translate.instant('salesQuotations.confirmText'),
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            this.salesQuotationService.confirm(event.item.id).subscribe({
              next: () => {
                this.loadData();
              }
            });
      ...
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          title: this.translate.instant('salesQuotations.cancelTitle'),
          text: this.translate.instant('salesQuotations.cancelText'),
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            this.salesQuotationService.cancel(event.item.id).subscribe({
              next: () => {
                this.loadData();
              }
            });
      ...
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          title: this.translate.instant('salesQuotations.closeTitle'),
          text: this.translate.instant('salesQuotations.closeText'),
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#f59e0b',
          cancelButtonColor: '#6b7280',
          confirmButtonText: this.translate.instant('salesQuotations.closeDocument'),
          cancelButtonText: this.translate.instant('common.cancel')
        }).then((result) => {
          if (result.isConfirmed) {
            this.salesQuotationService.close(event.item.id).subscribe({
              next: () => {
                this.loadData();
              }
            });
      ...
      import('sweetalert2').then(Swal => {
        Swal.default.fire({
          title: this.translate.instant('salesQuotations.convertTitle'),
          text: this.translate.instant('salesQuotations.convertText'),
          icon: 'info',
          showCancelButton: true,
          confirmButtonColor: '#3b82f6',
          cancelButtonColor: '#6b7280',
          confirmButtonText: this.translate.instant('salesQuotations.convertToInvoice'),
          cancelButtonText: this.translate.instant('common.cancel')
        }).then((result) => {
          if (result.isConfirmed) {
  ...
  openPrintModal(item: any): void {
    ...
    this.salesQuotationService.printPdf(item.id).subscribe({
      next: (blob) => {
