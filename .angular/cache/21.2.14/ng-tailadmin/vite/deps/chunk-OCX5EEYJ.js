...
__export(Utils_exports, {
  ...
  stripFormatTags: () => stripFormatTags,
  stripTags: () => stripTags,
  supports: () => supports,
  trim: () => trim,
  trimLeft: () => trimLeft,
...
var Settings = class {
  ...
  _onDebouncedHelper(map2, key, callback, delay) {
    let events = map2[key];
    if (events === void 0) {
      events = map2[key] = [];
    }
    ...
    return new Disposer(() => {
      ...
      removeFirst(events, entry);
      if (events.length === 0) {
        delete map2[key];
      }
    });
  }
  _offDebouncedHelper(map2, key, callback) {
    const events = map2[key];
    if (events !== void 0) {
      if (callback !== void 0) {
        const index = findIndex(events, (e) => e.cb === callback);
        if (index !== -1) {
  ...
  _sendKeyEvent(key, value) {
    ...
    if (debouncedEvents !== void 0) {
      each(debouncedEvents, (entry) => {
        if (entry.timeout) {
          window.clearTimeout(entry.timeout);
        }
...
//# sourceMappingURL=chunk-OCX5EEYJ.js.map
